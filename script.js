...
const code = 3270190217190;

async function analysisTags(productCode) {
  const product = await getInformation(productCode);
  const tagVeg = document.createElement('p');
  const produit = product.product;
  const tags = produit.ingredients_analysis_tags || [];
  const estVegan = tags.includes("en:vegan");
  const estVegetarien = tags.includes("en:vegetarian");

  if (estVegan) {
    tagVeg.innerHTML = "Ce produit est <strong>VEGAN</strong>";
    console.log(estVegan)
  } else if (estVegetarien) {
    tagVeg.innerHTML = "Ce produit est <strong>VÉGÉTARIEN</strong>";
  } else {
    tagVeg.innerHTML = "Ce produit n'est ni végétarien ni végan";
  }
  showInformation.appendChild(tagVeg);
}
